...
const PRODUCT = {
  ...
  details: ["Jugadores recomendados: 3–10", "Edad sugerida: 16+", "Duración: 20–60 min", "Idioma: Español"],
}
...
export default function ComprarPage() {
  ...
  const subtotal = useMemo(() => PRODUCT.priceBs * Math.max(1, qty), [qty])
  ...
  return (
    <main className="bg-[rgb(var(--bg))] text-[rgb(var(--fg))]">
      {/* Sin encabezado: va directo al producto */}
        ...
        <div className="grid gap-14 md:grid-cols-2">
                ...
                {PRODUCT.images.map((img, i) => (
                  <button
                    ...
                    className={`relative aspect-[4/3] overflow-hidden rounded-xl border transition ${
                      idx === i ? "border-black shadow-md" : "border-black/10 hover:border-black/40"
                    }`}
                    aria-label={`Ver imagen ${i + 1}`}
                  >
                    <Image fill sizes="120px" src={img.src} alt={img.alt} className="object-contain p-2" />
                  </button>
                ))}
              </div>
            </div>
          ...
          <motion.div {...fade}>
            <div className="rounded-2xl border border-black/10 bg-white p-8 sm:p-10 shadow-sm">
              <h2 className="text-[clamp(22px,3.4vw,30px)] font-extrabold">{PRODUCT.name}</h2>
              ...
              <div className="mt-4 text-sm text-black/70">
                Subtotal: <span className="font-semibold text-black">{formatBs(subtotal)}</span>
              </div>
              ...
              <div className="mt-6 grid gap-4 text-sm text-black/75 sm:grid-cols-2">
                <div className="rounded-xl border border-black/10 p-3">🚚 Envíos a todo Bolivia. Tracking y soporte por WhatsApp.</div>
                <div className="rounded-xl border border-black/10 p-3">💳 Pagos por transferencia/QR. Confirmación inmediata.</div>
              </div>
